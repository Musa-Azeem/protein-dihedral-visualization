...
class DihedralAdherence():
    ...
    def plot_one_dist(self, seq=None, pred_id=None, pred_name=None, axlims=None, bw_method=-1):
    ...
    def plot_one_dist_3d(self, seq=None, bw_method=-1):
        seq = seq or self.overlapping_seqs[0]
        plot_one_dist_3d(self, seq, bw_method)

    def plot_clusters_for_seq(self, seq=None, bw_method=-1):
        seq = seq or self.overlapping_seqs[0]
        plot_clusters_for_seq(self, seq, bw_method)

    def plot_md_for_seq(self, seq=None, pred_id=None, pred_name=None, axlims=None, bw_method=None):
        seq = seq or self.overlapping_seqs[0]
        pred_id = pred_id or self.protein_ids[0]
        plot_md_for_seq(self, seq, pred_id, pred_name, axlims, bw_method)
    
    def plot_res_vs_md(self, pred_id=None, pred_name=None, highlight_res=None, limit_quantile=None):
        ...
        return plot_res_vs_md(self, protein_id, pred_name, highlight_res, limit_quantile)
    
    def plot_md_vs_rmsd(self, axlims=None):
        if not 'md' in self.phi_psi_predictions.columns:
            print('No MD data available. Run compute_mds() or load_results_md() first')
            return
